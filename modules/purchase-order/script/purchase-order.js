...
let mainTable = 'OPOR';
let objectType = 0;
$('#pageTitle').text('Purchase Order | SAP B1');	
setTimeout(function()
	{
...
/*Load Tabs*/
	...
	//Logistics
	$('#logistics-tab').load('../templates/purchase-order-logistics.php'), function(){
		
	};
	//Accounting
	$('#accounting-tab').load('../templates/purchase-order-accounting.php'), function(){
...
//Submit
	...
	$(document.body).on('click', '#btnAdd', function () 
	{
        ...
        if (err == 0) 
		{
            ...
            $.ajax({
                type: 'POST',
                url: '../proc/exec/exec_add_purchase-order.php',
				data: 
				{
					json: json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
			    ...
			    success: function (data) 
				{
					...
					if(res.valid == true)
					{
						...
						$('#messageBar').text(res.msg).css({'background-color': '#00FF7F', 'color': 'black'});
							...
							window.location.replace("../templates/purchase-order-document.php");
							},3000)
					}
					else
					{
