...
$qry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."];
			SELECT DISTINCT
				...
				CASE WHEN T0.DocCur <> 'PHP' THEN (T0.DocTotalFC + T0.DiscSumFC) - T0.VatSumFC ELSE (T0.DocTotal + T0.DiscSum) - T0.VatSum + T0.WTSum END AS 'TotalBeforeDisc',
				...
				(SELECT SUM(Gross) FROM PCH9 WHERE PCH9.DocEntry = T0.DocEntry) AS TotalDownPayment
...
while (odbc_fetch_row($qry)) 
{
	$arr[] = array(
		"DocEntry" => odbc_result($qry, 'DocEntry'),
				...
				"TotalDownPayment" => number_format(odbc_result($qry, 'TotalDownPayment'),2),
