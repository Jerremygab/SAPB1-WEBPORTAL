...
let mainTable = 'OPQT';
let objType = 540000006;
$('#pageTitle').text('Purchase Quotation | SAP B1');	
setTimeout(function()
	{
...
/*Load Tabs*/
	//Contents
	$('#contents-tab').load('../templates/purchase-quotation-lines.php?serviceType=' + serviceType), function (){
		
	};
	//Logistics
	$('#logistics-tab').load('../templates/purchase-quotation-logistics.php'), function(){
		
	};
	//Accounting
	$('#accounting-tab').load('../templates/purchase-quotation-accounting.php'), function(){
...
//Submit
	...
	$(document.body).on('click', '#btnAdd', function () 
	{
        ...
        if (err == 0) 
		{
            ...
            $.ajax({
                type: 'POST',
                url: '../proc/exec/exec_add_purchase-quotation.php',
				data: 
				{
					json: json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
			    ...
			    success: function (data) 
				{
					...
					if(res.valid == true)
					{
						...
						$('#messageBar').text(res.msg).css({'background-color': '#00FF7F', 'color': 'black'});
							...
							window.location.replace("../templates/purchase-quotation-document.php");
							},3000)
					}
					else
					{
...
//Update	
	$(document.body).on('click', '#btnUpdate', function () 
	{
        ...
        if (err == 0) 
		{
            ...
            $.ajax({
                type: 'POST',
                url: '../proc/exec/exec_update_purchase-quotation.php',
				data: 
				{
					json: json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
					udfJson: udfJson.replace(/(\r\n|\n|\r)/gm, '[newline]'),
		...
		setTimeout(function () 
			{
						...
						$('#rowLoader').load('../templates/purchase-quotation-lines-row.php?serviceType=' + serviceType, function (result) 
						{
