$(document).ready(function () {
	let mainTable = 'OQUT';
	$('#pageTitle').text('Sales Quotation | SAP B1');
	...
	$(document.body).on('click', '#btnLastRecord', function (){
		...
		$.getJSON('../proc/views/vw_getLastEntry.php?table=' + table, function (data){
			...
			// $('#txtPostingDate2').prop('readonly',true);
			// $('#txtDeliveryDate2').prop('disabled',true);
			// $('#txtDeliveryDate2').prop('readonly',true);
			// $('#txtDocumentDate2').prop('disabled',true);
			// $('#txtDocumentDate2').prop('readonly',true);
			// $('#btnControlAccount').prop('disabled',true);
			// $('#btnControlAccount').prop('readonly',true);
			// $('#txtCustomerRefNo').prop('readonly',true);
			// $('#txtRemarks').prop('readonly',true);
			// $('#txtFooterDiscountPercentage').prop('readonly',true);
			// $('#txtFooterDiscountSum').prop('readonly',true);
		});
	});
	...
	/*Load Tabs*/
		//Contents
		$('#contents-tab').load('../templates/sales-quotation-lines.php?serviceType=' + serviceType), function (){
			
		};
		//Logistics
		$('#logistics-tab').load('../templates/sales-quotation-logistics.php'), function(){
			
		};
		//Accounting
		$('#accounting-tab').load('../templates/sales-quotation-accounting.php'), function(){
	...
	//Double Clicks
		...
		$(document.body).on('dblclick', '#tblBP tbody > tr', function () 
		{
			...
			$('#txtJournalMemo').val('Sales Quotations - ' + cardCode);
	...
	//On Shown Modals
		...
		$('#udfModal').on('shown.bs.modal',function(){
			let udfTable =  $('.selected-udf').find('input.inputUdf').attr('table');
				...
				$.ajax({
					type: 'GET',
					url: '../proc/views/vw_udfTable.php',
				...
				$.ajax({
					type: 'GET',
					url: '../proc/views/vw_udfTableDescription.php',
					data: {udfTable : udfTable},
					success: function (html) 
					{
	...
	let previousValue = '';
		...
		$(document.body).on('click', '#btnAdd', function () 
		{
			...
			var tbl = $('#tblDetails tbody tr').each(function (i) 
			{
				...
				if(serviceType == 'I'){
					if ($(this).find('input.itemcode').val() != ''){
						...
						itArr.push('"' + $(this).find('input.rowtotal').val().replace(/,/g, '') + '"');
			...
			if (err == 0) 
			{
				...
				$.ajax({
					type: 'POST',
					url: '../proc/exec/exec_add_sales-quotation.php',
					data: 
					{
						json: json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
					...
					success: function (data) 
					{
						...
						if(res.valid == true)
						{
							...
							$('#messageBar').text(res.msg).css({'background-color': '#00FF7F', 'color': 'black'});
								...
								window.location.replace("../templates/sales-quotation-document.php");
								},3000)
						}
						else
						{
	...
	//Update	
		$(document.body).on('click', '#btnUpdate', function () 
		{
			...
			if (err == 0) 
			{
				...
				$.ajax({
					type: 'POST',
					url: '../proc/exec/exec_update_sales-quotation.php',
					data: 
					{
						json: json.replace(/(\r\n|\n|\r)/gm, '[newline]'),
						udfJson: udfJson.replace(/(\r\n|\n|\r)/gm, '[newline]'),
	...
	//------------------------------------------------------------------------------------------------------------------------------------------------------------------		
			...
			setTimeout(function () 
				{
							...
							$('#rowLoader').load('../templates/sales-quotation-lines-row.php?serviceType=' + serviceType, function (result) 
							{
		...
		function PreviewDoc(docNum){
				...
				$('#btnAdd').addClass('d-none');
				$('#btnUpdate').removeClass('d-none');
				$('#selTransactionType').attr('disabled', true);
