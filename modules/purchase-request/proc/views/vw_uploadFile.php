		...
		if($i == $count-1){
			...
			if($type == 'add'){
				...
				if ($attachmentReturnVal == 0) {
					...
					$updateOPRQQry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."]; 
					UPDATE OPRQ SET AtcEntry = $atcentry WHERE DocEntry = $u_docentry
					
					");
					odbc_free_result($updateOPRQQry);
				}else{
