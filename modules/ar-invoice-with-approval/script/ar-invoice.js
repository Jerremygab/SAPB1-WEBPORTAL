$(document).ready(function () {
	...
	const copyFromArr = JSON.parse(sessionStorage.copyFromArr);
   	...
   	const ApprovalTemplateCode = $('#ApprovalTemplateCode').text();
   	const OriginatorEmpId = $('#OriginatorEmpId').text();
   	const ApprovalStageCode = $('#ApprovalStageCode').text();
   	const ApproverEmpId = $('#ApproverEmpId').text();
   	const ApprovalModules = $('#ApprovalModules').text();
   	const NoOfApprovals = $('#NoOfApprovals').text();
   	const NoOfDisapprovals = $('#NoOfDisapprovals').text();
	...
	$(document.body).on('click', '#btnFirstRecord', function () {
		...
		$.getJSON('../proc/views/vw_getFirstEntry.php?table=' + table, function (data) {
			docNum = data;
			PreviewDoc(docNum, objType, postedorDraft);
		});
	});
	...
	$(document.body).on('dblclick', '#tblDoc tbody > tr', function () {
		...
		var postedorDraft = $(this).children('td.item-8').text();
	...
	$(document.body).on('dblclick', '.copyFromTable tbody > tr', function () {
		...
		var docNum = $(this).children('td.item-1').text();
		var postedorDraft = $(this).children('td.item-6').text();
		var objType = baseType;
	...
	function PreviewDoc(docNum, objType, postedorDraft) {
		let docstatus = '';
		let docType = '';
		let docNumBuff;
		let table;
			...
			$.each(data, function (key, val) {
				...
				if (objType ==  val.ObjType) {
					...
					if (postedorDraft == 'DRAFT'){
						...
						$('#txtApprovalStatus').css({ 'background-color': 'none'});
					} else {
	...
	function PreviewUDF(docNum, objType) {
		...
		$.getJSON('../proc/views/udf/vw_listUDF.php?mainTable=' + mainTable, function (data) {
			...
			$.each(data, function (key, val) {
				...
				// udfArr.push('CAST(' + val.Column_Name + ' AS NVARCHAR(200)) AS ' + val.Column_Name);
				udfArr.join(',');
			});
