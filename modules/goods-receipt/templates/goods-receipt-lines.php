...
<table id="tblDetails" class="table table-striped table-bordered table-sm detailsTable" cellspacing="0"  style="background-color: white; width= 100%;" cellspacing="0" >
  <thead   style="border-bottom: 0 !important">
    ...
    <th style="color: black; min-width:200px;">UoM Code</th>
	  ...
	  <th style="color: black; min-width:200px;">Acct Code</th>
    ...
    <th style="color: black; min-width:150px;" class="d-none">Tax Code</th>
	  <th style="color: black; min-width:150px;" class="d-none">Gross Price</th>
    <th style="color: black; min-width:180px;">Total</th>
	  <th style="color: black; min-width:180px;" class="d-none">Gross Total</th>
    ...
    <tr style="background-color: white; "  >
	 <td class="rowno text-right" style="background-color: lightgray;color:black; font-size:13px;">
      ...
      <td >
		...
		<input type="text" class="form-control batchorserialexpdate"  aria-label="Recipient's username" aria-describedby="button-addon2" style="outline: none; border:none; " readonly/>
	  ...
	  <td >
		<div class="input-group ">
		<input type="text" class="form-control matrix-cell itemname"  aria-label="Recipient's username" aria-describedby="button-addon2" style="outline: none; border:none" readonly/>
		  <button class="btn "   type="button" data-mdb-ripple-color="dark"  style="background-color: #ADD8E6; "  data-toggle="modal" data-target="#itemModal" data-backdrop="false">
			<i class="fas fa-list-ul input-prefix" tabindex=0 style="color:blue " ></i>
		  </button>
		<input type="hidden" class="form-control matrix-cell uomgroup"  aria-label="Recipient's username" aria-describedby="button-addon2" style="outline: none; border:none" readonly/>
							...
							<?php
												$qry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."]; SELECT Code,Name,Rate FROM OVTG WHERE Inactive = 'N' AND Category='I' ORDER BY CASE WHEN Code = 'IVAT-N' THEN '1' ELSE Code END ASC");
													while (odbc_fetch_row($qry)) 
													{
														...
														echo '<option  class="taxoptions" val-rate="0" value="' . odbc_result($qry, "Code") . '"  >' . odbc_result($qry, "Code") . '</option>';
													}
