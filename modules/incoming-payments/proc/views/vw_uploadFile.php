...
UPDATE ORCT SET CounterRef = '$txtReference', Comments = '$txtRemarks' 
WHERE DocEntry = $u_docentry
		...
		if($i == $count-1){
			...
			if($type == 'add'){
				...
				if ($attachmentReturnVal == 0) {
					...
					$updateOINVQry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."]; 
					UPDATE ORCT SET AtcEntry = $atcentry WHERE DocEntry = $u_docentry
					
					");
					odbc_free_result($updateOINVQry);
				}else{
