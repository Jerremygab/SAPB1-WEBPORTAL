        ...
        foreach ($rows as $row) {
            ...
            foreach ($row->props as $key => $value) {
                if (get_class($caller) === 'BillingTab' 
                    ...
                    $this->sendMessage(
                        $model->getSettings()->notificationReceipientUserCode, 
            ...
            foreach ($relatedTables as $relatedTable) {
                ...
                foreach ($rows as $row) {
                    ...
                    foreach ($row->props as $key => $value) {
                        ...
                        if ((bool)$caller->foreignFields && (!in_array($key, $caller->foreignFields) || (isset($row->forceUpdateFields) && in_array($key, $row->forceUpdateFields)))) {
