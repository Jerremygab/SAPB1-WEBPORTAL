...
BEGIN
    ...
    SELECT
        ...
        CAST(POD.U_ActualDateRec_Intitial AS DATE) AS U_ActualDateRec_Intitial,
        POD.U_SAPTrucker,
        POD.U_PlateNumber,
        POD.U_VehicleTypeCap,
        POD.U_DeliveryStatus,
        CAST(POD.U_DeliveryDateDTR AS DATE) AS U_DeliveryDateDTR,
        CAST(POD.U_DeliveryDatePOD AS DATE) AS U_DeliveryDatePOD,
        POD.U_NoOfDrops,
        POD.U_TripType,
        POD.U_Receivedby,
        CAST(POD.U_ClientReceivedDate AS DATE) AS U_ClientReceivedDate,
        CAST(POD.U_InitialHCRecDate AS DATE) AS U_InitialHCRecDate,
        CAST(POD.U_ActualHCRecDate AS DATE) AS U_ActualHCRecDate,
        CAST(POD.U_DateReturned AS DATE) AS U_DateReturned,
        POD.U_PODinCharge,
        CAST(POD.U_VerifiedDateHC AS DATE) AS U_VerifiedDateHC,
        POD.U_PTFNo,
        CAST(POD.U_DateForwardedBT AS DATE) AS U_DateForwardedBT,
        CAST(POD.U_BillingDeadline AS DATE) AS U_BillingDeadline,
        CASE WHEN EXISTS(
            SELECT TOP 1
            CASE 
                    WHEN EXISTS (
