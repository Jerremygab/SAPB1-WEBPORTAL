...
END AS DisableSomeFields,
	...
	CAST(T0.U_ShipmentManifestNo as nvarchar(200)) U_ShipmentManifestNo,
	...
	ISNULL(T0.U_GrossInitialRate,0) AS U_GrossInitialRate,
	ISNULL(T0.U_TotalAddtlCharges,0) AS U_GrossInitialRate,
	ISNULL(T0.U_ActualBilledRate,0) AS U_ActualBilledRate,
	ISNULL(T0.bi_U_RateAdjustments,0) AS U_RateAdjustments,
	ISNULL(T0.bi_U_ActualDemurrage,0) AS U_ActualDemurrage,
	...
	CAST(ISNULL(T0.U_GrossInitialRate,0) AS float) +
	CAST(ISNULL(T0.U_Demurrage,0) AS float) +
	CAST(ISNULL(T0.U_TotalAddtlCharges,0) AS float) +
	--(CAST(ISNULL(T0.U_AddtlDrop2,0) AS float) +
		--CAST(ISNULL(T0.U_BoomTruck,0) AS float) +
		--CAST(ISNULL(T0.U_Manpower2,0) AS float) +
		--CAST(ISNULL(T0.U_Backload2,0) AS float) ) +
	CAST(ISNULL(T0.U_ActualBilledRate,0) AS float) +
	CAST(ISNULL(T0.bi_U_RateAdjustments,0) AS float) +
	CAST(ISNULL(T0.bi_U_ActualDemurrage,0) AS float) + 
	CAST(ISNULL(T0.U_ActualAddCharges,0) AS float) AS U_TotalRecClients,
	...
	--ISNULL(T0.U_Backload,0)  AS U_AddCharges,
	...
	CAST(T0.U_OtherPODDoc as nvarchar(200)) U_OtherPODDoc,
...
WHERE H.CANCELED = 'N' AND L.ItemCode = T0.U_BookingId) - T0.U_TotalRecClients AS U_VarAR
	...
	while (odbc_fetch_row($qry)) 
	{
		...
		"U_PODNum"=> utf8_encode(odbc_result($qry, 'bi_U_PODNum')),
