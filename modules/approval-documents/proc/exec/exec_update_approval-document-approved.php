...
$approvedbyapprovers =  preg_replace('/(\v|\s)+/', '', $_POST['approvedbyapprovers']); 
...
$SAPDocNumUpdated = '';
if ($approvedbyapprovers == '') {
    $approved  = $EmpId;
}
else{
		if (!str_contains($approvedbyapprovers, $EmpId)) { 
			$approved = $approvedbyapprovers .= $approver;
		}
		else{
		$approved = $approvedbyapprovers;
	}
    
}
// echo "approvedbyapprovers: ".$approvedbyapprovers;
$approvedNoUnwantedSpace = preg_replace('/(\v|\s)+/', '', $approved);
$approvedbyapproversNoUnwantedSpace = preg_replace('/(\v|\s)+/', '', $approvedbyapprovers);
$noapproversrequiredNoUnwantedSpace = preg_replace('/(\v|\s)+/', '', $noapproversrequired);


$docentryqry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB2."]; 
    UPDATE [USER-COMMON].[dbo].[SAP_DOCS_WITH_APPROVAL] SET ApprovedByApprovers = '$approvedNoUnwantedSpace'
		...
		if( count($result2) == $NoOfDisapprovalsRequired){
			...
			while (odbc_fetch_row($qry)) 
			{
				...
				$oRdr->Lines->Add();
						...
						{
							$vCmp->GetNewObjectCode($SAPDocNumUpdated);
								
								$docentryqry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."]; 
									SELECT DocNum FROM OINV WHERE DocEntry = '$SAPDocNumUpdated'");
									$NewDocNum = odbc_result($docentryqry, "DocNum");


								$docentryqry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB2."]; 
									UPDATE [USER-COMMON].[dbo].[SAP_DOCS_WITH_APPROVAL] SET SAPDocEntry = '$NewDocNum'
