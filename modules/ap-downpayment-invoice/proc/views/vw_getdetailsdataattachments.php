...
$qry = odbc_exec($MSSQL_CONN, "USE [".$MSSQL_DB."];
	...
	FROM [@ATTACHMENT] T0 
	INNER JOIN ODPO T1 ON T1.DocEntry = T0.U_DocEntry


	WHERE T1.DocNum IN ( $docNum ) AND T0.U_ObjectType = 204
...
<input type="text" class="form-control matrix-cell targetpath"  aria-label="File Name"aria-describedby="button-addon2" style="outline: none; border:none" readonly value="'.$U_FileLocation.'"/>
	  ...
	  <td >
		<div class="input-group ">
			<button  href="../../../uploaded-files/'.$U_FileName.'" download="'.$U_FileName.'" class="pdf btn btn-sm">
				'.$U_FileName.'
			</button>						
		</div>
	  </td>
